...
### Fixed
- **What Home Assistant had already set when Scribe started was never recorded**: Scribe only ever recorded *changes*, through a listener registered once it is set up — and Home Assistant is well into its start by then. An entity that changed while Home Assistant was down and did not change again afterwards was simply missing from the history, which went on showing the previous value across the gap; Scribe's own connectivity sensor, set up before the listener existed, had not been recorded since the last time it changed. Scribe now registers its listener before its platforms, and records the state of everything already set, once, at startup. What that adds depends on why Scribe is starting. After a **Home Assistant restart**, Home Assistant has given every entity a fresh timestamp, so these are new rows — one per entity, measured at 1361 rows on an installation of 795 live entities, against 58 000 recorded in a day. After a **Scribe reload** (saving the options), the states still carry the timestamps they already had, so each row is the one already stored and the primary key on `(metadata_id, time)` drops it. Nothing is ever written twice. The exception is a database created by Scribe 3.1 to 3.5, which has no such key (no release ever added it): there the startup states are skipped rather than duplicated at every restart, and a line in the log says so.
