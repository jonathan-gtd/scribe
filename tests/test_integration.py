...
def disable_mock_pool():
    ...
    # Patch the writer's asyncpg.create_pool
...
async def setup_db_for_integration(url):
    ...
    try:
        ...
        _LOGGER.warning(f"DB setup failed (might already be OK): {e}")
    finally:
        await conn.close()
...
async def log_db_state(url):
    ...
    try:
        ...
        _LOGGER.info(f"Tables/Views: {[(t['table_name'], t['table_type']) for t in tables]}")
...
async def test_integration_write_and_read(hass, socket_enabled):
    ...
    try:
        ...
        try:
            ...
            # Force cache reload on writer if needed, but it should happen in start()
        finally:
            await conn.close()
        ...
        # Wait for flush (increased to 5s for CI)
